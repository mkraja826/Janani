...
export default function AiCompanionScreen() {
  ...
  async function submit(category: CarePlusAiCategory, text?: string) {
    ...
    try { const pregnancyId = await resolveActivePregnancyId(userId); if (!pregnancyId) throw new Error('No active pregnancy was found.'); const result = await requestCarePlusAi({ pregnancyId, category, userText: text });
      if (result.text) setAnswer(result.text); else if (result.error === 'care_plus_required') setAnswer('Janani Care+ requires an active subscription. Your free Janani features continue to work normally.'); else if (result.error === 'condition_rule_pack_not_approved') setAnswer('Janani is not yet clinically approved to personalise this request for one or more of your saved conditions. Please follow your maternity team’s individual guidance.'); else if (result.error === 'ai_temporarily_unavailable' || result.error === 'ai_provider_disabled') setAnswer('Janani Care+ is temporarily unavailable. Your saved health information and other Janani features are unaffected.'); else if (result.error === 'care_plus_quota_unavailable') setAnswer('Your current Care+ AI allowance is unavailable or has been reached.'); else setAnswer('Janani Care+ could not prepare this safely right now.');
    } catch (error) { Alert.alert('Janani Care+ is unavailable', error instanceof Error ? error.message : 'Please try again later.'); } finally { setLoading(false); }
  }
  ...
  return <SafeAreaView style={styles.page}><KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}><ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
    ...
    {answer ? <View style={styles.answerCard}><View style={[styles.answerHeader,rtl.row]}><View style={styles.answerIcon}><Ionicons name="heart" size={20} color={colors.rose} /></View><Text style={[styles.answerTitle,rtl.startText]}>Janani Care+</Text></View><Text selectable style={[styles.answerText,rtl.startText]}>{answer}</Text></View> : null}
