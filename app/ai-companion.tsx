...
export default function AiCompanionScreen() {
  ...
  async function submit(category: CarePlusAiCategory, text?: string) {
    ...
    try {
      ...
      else if (result.error === 'care_plus_required') setAnswer('Janani Care+ requires an active subscription. Your free Janani features continue to work normally.');
      else if (result.error === 'condition_rule_pack_not_approved') setAnswer('Janani is not yet clinically approved to personalise this request for one or more of your saved conditions. Please follow your maternity team’s individual guidance.');
      else if (result.error === 'ai_temporarily_unavailable' || result.error === 'ai_provider_disabled') setAnswer('Janani Care+ is temporarily unavailable. Your saved health information and other Janani features are unaffected.');
      ...
      else setAnswer('Janani Care+ could not prepare this safely right now.');
    } catch (error) {
  ...
  return <SafeAreaView style={styles.page}><KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}><ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
    <View style={styles.header}><Pressable accessibilityLabel="Go back" onPress={() => router.back()} style={styles.backButton}><Ionicons name="arrow-back" size={22} color={colors.ink} /></Pressable><View style={styles.headerCopy}><Text style={styles.eyebrow}>JANANI CARE+</Text><Text style={styles.title}>{copy.carePlusTitle}</Text></View></View>
    <View style={styles.safetyCard}><Ionicons name="shield-checkmark-outline" size={25} color={colors.rose} /><Text style={styles.safetyText}>Care+ uses only relevant Janani context for each request. It cannot diagnose, prescribe, change medicines, set medical targets, or confirm that you or your baby are safe.</Text></View>
    <View style={styles.quickRow}>{quickActions.map((item) => <Pressable key={item.category} disabled={loading} onPress={() => void submit(item.category)} style={styles.quickAction}><Ionicons name={item.icon} size={21} color={colors.rose} /><Text style={styles.quickText}>{item.label}</Text></Pressable>)}</View>
    <View style={styles.inputCard}><Text style={styles.label}>{copy.askCarePlus}</Text><TextInput multiline maxLength={1200} onChangeText={setMessage} placeholder={copy.placeholder} placeholderTextColor={colors.muted} style={styles.input} value={message} /><Pressable disabled={!message.trim() || loading} onPress={() => void submit('explain_guidance', message)} style={[styles.askButton, (!message.trim() || loading) && styles.askButtonDisabled]}>{loading ? <ActivityIndicator color={colors.surface} /> : <Ionicons name="sparkles-outline" size={19} color={colors.surface} />}<Text style={styles.askButtonText}>{loading ? copy.preparing : copy.ask}</Text></Pressable></View>
