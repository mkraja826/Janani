...
export default function RemindersScreen() {
  ...
  const load = useCallback(async () => {
    ...
    if (items.error || history.error) Alert.alert('Could not load reminders', items.error?.message ?? history.error?.message);
      ...
      setReminders((items.data ?? []).filter((item) => item.days_of_week.length === 0 || item.days_of_week.includes(weekday)) as Reminder[]);
      setLogs((history.data ?? []) as ReminderLog[]);
    }
  ...
  async function mark(reminder: Reminder, state: 'taken' | 'skipped') {
    ...
    const { error } = await supabase.rpc('mark_reminder_occurrence', { p_reminder_id: reminder.id, p_scheduled_for: scheduledFor, p_state: state, p_note: null });
  ...
  return <SafeAreaView style={styles.page}>
    <View style={styles.header}>
      ...
      <View style={styles.headerCopy}><Text style={styles.eyebrow}>TODAY’S CARE</Text><Text style={styles.title}>Gentle reminders</Text></View>
      <Pressable accessibilityLabel="Add reminder" onPress={() => router.push('/new-reminder')} style={styles.addButton}><Ionicons name="add" size={24} color={colors.surface} /></Pressable>
    </View>
