...
export default function HealthTrackerScreen() {
  ...
  const rtl = rtlLayoutFor(language);
  ...
  function number(value: string, min: number, max: number, label: string) { const parsed = Number(value); if (!value.trim() || !Number.isFinite(parsed) || parsed < min || parsed > max) { Alert.alert(`Check ${label}`, `Enter a value between ${min} and ${max}.`); return null; } return parsed; }
  ...
  return <SafeAreaView style={styles.page}><ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
    <View style={[styles.header, rtl.row]}><Pressable accessibilityLabel="Back" onPress={()=>router.back()} style={styles.icon}><Ionicons name={directionalIconName(language,'arrow-back','arrow-forward') as keyof typeof Ionicons.glyphMap} size={22} color={colors.ink}/></Pressable><View style={styles.flex}><Text style={[styles.eyebrow, rtl.startText]}>{tr('trackerEyebrow')}</Text><Text style={[styles.title, rtl.startText]}>{tr('trackerTitle')}</Text></View></View>
    <View style={[styles.notice, rtl.row]}><Ionicons name="information-circle-outline" size={22} color={colors.roseDark}/><Text style={[styles.noticeText, rtl.startText]}>Janani records what you enter. It does not diagnose a condition or decide whether a reading is safe.</Text></View>
    <View style={[styles.tabs, rtl.row]}>{kinds.map((item)=><Pressable key={item.value} onPress={()=>{setKind(item.value);reset();}} style={[styles.tab,kind===item.value&&styles.tabSelected]}><Text style={[styles.tabText,kind===item.value&&styles.tabTextSelected,rtl.startText]}>{item.label}</Text></Pressable>)}</View>
    <View style={styles.card}><Text style={[styles.cardTitle,rtl.startText]}>{tr('add')} {kindLabel}</Text><View style={styles.form}>
      {kind==='weight'&&<Field rtlText={rtl.startText} label={`${tr('weight')} (kg)`} value={a} onChangeText={setA} keyboardType="decimal-pad" placeholder="62.5"/>}
      {kind==='blood_pressure'&&<><Field rtlText={rtl.startText} label={tr('systolic')} value={a} onChangeText={setA} keyboardType="number-pad"/><Field rtlText={rtl.startText} label={tr('diastolic')} value={b} onChangeText={setB} keyboardType="number-pad"/><Field rtlText={rtl.startText} label={tr('pulseOptional')} value={c} onChangeText={setC} keyboardType="number-pad"/><Field rtlText={rtl.startText} label={tr('symptomsOptional')} value={d} onChangeText={setD} placeholder="Headache, dizziness"/></>}
      {kind==='glucose'&&<><Field rtlText={rtl.startText} label={tr('glucoseMgDl')} value={a} onChangeText={setA} keyboardType="decimal-pad"/><View style={[styles.tabs,rtl.row]}>{(['fasting','before_meal','after_meal','random','other'] as const).map((x)=><Pressable key={x} onPress={()=>setContext(x)} style={[styles.tab,context===x&&styles.tabSelected]}><Text style={[styles.tabText,context===x&&styles.tabTextSelected,rtl.startText]}>{contextLabel(x)}</Text></Pressable>)}</View>{context==='after_meal'?<Field rtlText={rtl.startText} label={tr('minutesAfterMeal')} value={b} onChangeText={setB} keyboardType="number-pad"/>:null}</>}
      {kind==='lab'&&<><Field rtlText={rtl.startText} label={tr('testName')} value={a} onChangeText={setA}/><Field rtlText={rtl.startText} label={tr('result')} value={b} onChangeText={setB}/><Field rtlText={rtl.startText} label={tr('unitOptional')} value={c} onChangeText={setC}/><Field rtlText={rtl.startText} label={tr('referenceRangeOptional')} value={d} onChangeText={setD}/></>}
      {kind==='symptom'&&<><Field rtlText={rtl.startText} label={tr('symptom')} value={a} onChangeText={setA}/><Text style={[styles.label,rtl.startText]}>{tr('severity')}</Text><View style={[styles.tabs,rtl.row]}>{[1,2,3,4,5].map((x)=><Pressable key={x} onPress={()=>setSeverity(x)} style={[styles.tab,severity===x&&styles.tabSelected]}><Text style={[styles.tabText,severity===x&&styles.tabTextSelected]}>{x}</Text></Pressable>)}</View><Field rtlText={rtl.startText} label={tr('durationMinutesOptional')} value={b} onChangeText={setB} keyboardType="number-pad"/><Pressable onPress={()=>setContactedCare((v)=>!v)} style={[styles.check,rtl.row]}><Ionicons name={contactedCare?'checkbox':'square-outline'} size={22} color={colors.roseDark}/><Text style={[styles.checkText,rtl.startText]}>{tr('contactedCareTeam')}</Text></Pressable></>}
      <Field rtlText={rtl.startText} label={tr('noteOptional')} value={note} onChangeText={setNote} multiline maxLength={500}/><Pressable disabled={saving} onPress={()=>void save()} style={[styles.save,saving&&styles.disabled]}>{saving?<ActivityIndicator color={colors.surface}/>:<Text style={styles.saveText}>{tr('saveEntry')}</Text>}</Pressable>
    </View></View>
    <View style={styles.card}><Text style={[styles.cardTitle,rtl.startText]}>{tr('recentEntries')}</Text>{recent.length===0?<Text style={[styles.empty,rtl.startText]}>{tr('noEntries')}</Text>:recent.map((item)=><View key={item.id} style={[styles.entry,rtl.row]}><View style={styles.flex}><Text style={[styles.entryTitle,rtl.startText]}>{item.title}</Text><Text style={[styles.entryMeta,rtl.startText]}>{item.meta}</Text></View><Pressable accessibilityLabel={tr('deleteEntry')} onPress={()=>remove(item.id)}><Ionicons name="trash-outline" size={19} color={colors.muted}/></Pressable></View>)}</View>
...
function Field(props:React.ComponentProps<typeof TextInput>&{label:string;rtlText?:object}){const{label,rtlText,...input}=props;return <View style={styles.field}><Text style={[styles.label,rtlText]}>{label}</Text><TextInput {...input} placeholderTextColor={colors.muted} style={[styles.input,rtlText,input.multiline&&styles.multiline]}/></View>}
