...
export default function HealthProfileScreen() {
  ...
  async function save() {
    ...
    try {
      ...
      Alert.alert('Health profile saved', 'These details remain private to the mother unless PregaLove explicitly says otherwise.');
  ...
  return <SafeAreaView style={styles.page}><ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
    <View style={[styles.header, rtl.row]}><Pressable accessibilityLabel="Back" onPress={() => router.back()} style={styles.iconButton}><Ionicons name={directionalIconName(language, 'arrow-back', 'arrow-forward') as keyof typeof Ionicons.glyphMap} size={22} color={colors.ink} /></Pressable><View style={styles.flex}><Text style={[styles.eyebrow, rtl.startText]}>{tr('profileEyebrow')}</Text><Text style={[styles.title, rtl.startText]}>{tr('profileTitle')}</Text></View></View>
    <View style={[styles.notice, rtl.row]}><Ionicons name="lock-closed-outline" size={22} color={colors.roseDark} /><Text style={[styles.noticeText, rtl.startText]}>These details are mother-only by default. PregaLove records what you report and does not diagnose a condition or change treatment.</Text></View>
    <Section title={tr('pregnancyBody')} rtlText={rtl.startText}><Field rtlText={rtl.startText} label={tr('currentWeight')} value={weight} onChangeText={setWeight} keyboardType="decimal-pad" placeholder="62.5" /><Choice rtlText={rtl.startText} rtlRow={rtl.row} label={tr('pregnancy')} items={pregnancyTypes} value={pregnancyType} onChange={setPregnancyType} /><Choice rtlText={rtl.startText} rtlRow={rtl.row} label={tr('activityLevel')} items={activities} value={activity} onChange={setActivity} /></Section>
    <Section title={tr('foodPreferences')} rtlText={rtl.startText}><Choice rtlText={rtl.startText} rtlRow={rtl.row} label={tr('foodPattern')} items={diets} value={diet} onChange={setDiet} /><Field rtlText={rtl.startText} label={tr('cuisinePreferences')} value={cuisines} onChangeText={setCuisines} placeholder="Telangana, South Indian" /><Field rtlText={rtl.startText} label={tr('foodAllergies')} value={allergies} onChangeText={setAllergies} placeholder="Peanut, lactose" /><Field rtlText={rtl.startText} label={tr('foodsAvoided')} value={foodsAvoided} onChangeText={setFoodsAvoided} placeholder="Mushrooms, seafood" /></Section>
    <Section title={tr('healthConditions')} subtitle={`${selectedCount} selected. PregaLove will never infer a diagnosis from your readings.`} rtlText={rtl.startText}>{CONDITION_OPTIONS.map((item) => <View key={item.code} style={styles.condition}><Text style={[styles.conditionTitle, rtl.startText]}>{item.label}</Text><View style={[styles.row, rtl.row]}>{statuses.filter((status) => !item.historyOnly || status.value === 'pregnancy_history').map((status) => { const selected = conditions[item.code] === status.value; return <Pressable key={status.value} onPress={() => setConditions((current) => ({ ...current, [item.code]: selected ? undefined : status.value }))} style={[styles.pill, selected && styles.pillSelected]}><Text style={[styles.pillText, selected && styles.pillTextSelected, rtl.startText]}>{status.label}</Text></Pressable>; })}</View></View>)}</Section>
    <Section title={tr('clinicianInstructions')} subtitle="Doctor or dietitian instructions always take priority over generic PregaLove guidance." rtlText={rtl.startText}><Field rtlText={rtl.startText} label={tr('dietaryActivityInstructions')} value={instructions} onChangeText={setInstructions} placeholder="Enter only instructions your care team has given you" multiline maxLength={2000} /></Section>
    <Pressable disabled={saving} onPress={() => void save()} style={[styles.save, rtl.row, saving && styles.disabled]}>{saving ? <ActivityIndicator color={colors.surface} /> : <><Text style={styles.saveText}>{tr('saveHealthProfile')}</Text><Ionicons name="shield-checkmark-outline" size={20} color={colors.surface} /></>}</Pressable>
...
function Section({ title, subtitle, children, rtlText }: { title: string; subtitle?: string; children: React.ReactNode; rtlText?: object }) { return <View style={styles.card}><Text style={[styles.cardTitle, rtlText]}>{title}</Text>{subtitle ? <Text style={[styles.subtitle, rtlText]}>{subtitle}</Text> : null}<View style={styles.sectionBody}>{children}</View></View>; }
function Field({ label, rtlText, ...props }: React.ComponentProps<typeof TextInput> & { label: string; rtlText?: object }) { return <View style={styles.field}><Text style={[styles.label, rtlText]}>{label}</Text><TextInput {...props} placeholderTextColor={colors.muted} style={[styles.input, rtlText, props.multiline && styles.multiline]} /></View>; }
function Choice<T extends string>({ label, items, value, onChange, rtlText, rtlRow }: { label: string; items: Array<{ value: T; label: string }>; value: T; onChange: (value: T) => void; rtlText?: object; rtlRow?: object }) { return <View style={styles.field}><Text style={[styles.label, rtlText]}>{label}</Text><View style={[styles.row, rtlRow]}>{items.map((item) => <Pressable key={item.value} onPress={() => onChange(item.value)} style={[styles.pill, item.value === value && styles.pillSelected]}><Text style={[styles.pillText, item.value === value && styles.pillTextSelected, rtlText]}>{item.label}</Text></Pressable>)}</View></View>; }
