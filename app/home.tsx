...
export default function HomeScreen() {
  ...
  useEffect(()=>{ if(!session){router.replace('/auth');return;} (async()=>{
    ...
    const list=Array.isArray(family?.pregnancies)?family?.pregnancies:family?.pregnancies?[family.pregnancies]:[];
    const pregnancy=list.find((item)=>item.status==='active')??list[0];
    setSummary({role:data.role,familyName:family?.name??'Our little family',dueDate:pregnancy?.due_date??null,inviteCode:data.role==='mother'?family?.invite_code??null:null});setLoading(false);
  })();},[session]);
