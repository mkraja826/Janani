...
export default function HomeScreen() {
  ...
  return <SafeAreaView style={styles.page}><ScrollView contentContainerStyle={styles.content}>
    <View style={styles.topRow}><View style={styles.flex}><Text style={styles.eyebrow}>{summary?.familyName.toUpperCase()}</Text><Text style={styles.title}>{summary?.role==='mother'?'How are you feeling today?':'A little care goes a long way.'}</Text></View><Pressable accessibilityLabel="Sign out" onPress={async()=>{await signOut();router.replace('/');}} style={styles.iconButton}><Ionicons name="log-out-outline" size={22} color={colors.muted}/></Pressable></View>
