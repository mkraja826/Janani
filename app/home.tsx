...
export default function HomeScreen() {
  ...
  return <SafeAreaView style={styles.page}><ScrollView contentContainerStyle={styles.content}>
    <View style={[styles.topRow, rtl.row]}><View style={styles.flex}><Text style={[styles.eyebrow, rtl.startText]}>{summary?.familyName.toUpperCase()}</Text><Text style={[styles.title, rtl.startText]}>{tr(isMother ? 'homeGreetingMother' : 'homeGreetingPartner')}</Text></View><View style={[styles.topActions, rtl.row]}><Pressable accessibilityLabel="Settings" onPress={() => router.push('/settings')} style={styles.iconButton}><Ionicons name="settings-outline" size={21} color={colors.muted}/></Pressable><Pressable accessibilityLabel="Sign out" onPress={() => void finishSignOut()} style={styles.iconButton}><Ionicons name="log-out-outline" size={22} color={colors.muted}/></Pressable></View></View>
    <Pressable onPress={() => router.push('/pregnancy-guide')} style={[styles.heroCard, rtl.row]}><Ionicons name="heart-circle" size={52} color={colors.rose}/><View style={styles.flex}><Text style={[styles.cardEyebrow, rtl.startText]}>{tr('todayWithJanani')}</Text>{progress ? <><Text style={[styles.week, rtl.startText]}>{tr('week')} {progress.gestationalWeek}</Text><Text style={[styles.cardTitle, rtl.startText]}>{trimesterLabel(progress.trimester)} · {tr('day')} {progress.gestationalDay}</Text><Text style={[styles.cardMeta, rtl.startText]}>{progress.isPastDue ? 'Your due date has arrived. Keep in touch with your maternity care team.' : `${progress.daysRemaining} days until the estimated due date`}</Text><Text style={[styles.heroLink, rtl.startText]}>{tr('openPregnancyGuide')}</Text></> : <><Text style={[styles.cardTitle, rtl.startText]}>{tr('hydrationPrompt')}</Text><Text style={[styles.heroLink, rtl.startText]}>{tr('openPregnancyGuide')}</Text></>}</View></Pressable>
    {summary?.inviteCode ? <View style={styles.inviteCard}><Text style={[styles.inviteLabel, rtl.startText]}>{tr('partnerInviteCode')}</Text><Text selectable style={[styles.inviteCode, rtl.startText]}>{summary.inviteCode}</Text><Text style={[styles.inviteHelp, rtl.startText]}>{tr('partnerInviteHelp')}</Text></View> : null}
    ...
    <View style={[styles.grid, rtl.row]}>
      ...
      {showCarePlus ? <Feature rtl={rtl.isRtl} icon="sparkles-outline" title={tr('carePlus')} caption={tr(productionConfig.aiUiEnabled ? 'carePlusCaption' : 'carePlusOfflineCaption')} onPress={() => router.push('/ai-companion')} /> : null}
...
function Feature({icon,title,caption,onPress,rtl}:{icon:keyof typeof Ionicons.glyphMap;title:string;caption:string;onPress?:()=>void;rtl:boolean}) { const textStyle={textAlign:rtl?'right':'left',writingDirection:rtl?'rtl':'ltr'} as const; return <Pressable disabled={!onPress} onPress={onPress} style={[styles.feature,!onPress&&styles.featureDisabled]}><View style={[styles.featureIcon,rtl&&{alignSelf:'flex-end'}]}><Ionicons name={icon} size={24} color={colors.rose}/></View><Text style={[styles.featureTitle,textStyle]}>{title}</Text><Text style={[styles.featureCaption,textStyle]}>{caption}</Text></Pressable>; }
function BackgroundFeature({icon,text,rtl}:{icon:keyof typeof Ionicons.glyphMap;text:string;rtl:boolean}) { return <View style={[styles.backgroundRow,rtl&&{flexDirection:'row-reverse'}]}><Ionicons name={icon} size={19} color={colors.sage}/><Text style={[styles.backgroundText,{textAlign:rtl?'right':'left',writingDirection:rtl?'rtl':'ltr'}]}>{text}</Text></View>; }
const styles=StyleSheet.create({page:{flex:1,backgroundColor:colors.background},center:{flex:1,alignItems:'center',justifyContent:'center',gap:spacing.md,padding:spacing.xl,backgroundColor:colors.background},content:{padding:spacing.lg,paddingBottom:spacing.xxl,gap:spacing.xl},topRow:{flexDirection:'row',justifyContent:'space-between',gap:spacing.md,alignItems:'flex-start'},topActions:{flexDirection:'row',gap:spacing.sm},flex:{flex:1},eyebrow:{fontSize:12,letterSpacing:2.2,fontWeight:'800',color:colors.rose},title:{marginTop:spacing.sm,maxWidth:290,fontSize:30,lineHeight:37,fontWeight:'800',color:colors.ink},iconButton:{width:44,height:44,borderRadius:radius.pill,backgroundColor:colors.surface,alignItems:'center',justifyContent:'center',borderWidth:1,borderColor:colors.border},heroCard:{flexDirection:'row',gap:spacing.md,padding:spacing.lg,borderRadius:radius.lg,backgroundColor:colors.blush,borderWidth:1,borderColor:colors.border},cardEyebrow:{fontSize:11,letterSpacing:1.8,fontWeight:'800',color:colors.roseDark},week:{marginTop:spacing.sm,fontSize:30,fontWeight:'900',color:colors.ink},cardTitle:{marginTop:4,fontSize:17,lineHeight:24,fontWeight:'700',color:colors.ink},cardMeta:{marginTop:spacing.md,fontSize:13,lineHeight:19,color:colors.muted},heroLink:{marginTop:spacing.md,fontSize:13,fontWeight:'800',color:colors.roseDark},inviteCard:{padding:spacing.lg,borderRadius:radius.lg,backgroundColor:colors.surface,borderWidth:1,borderColor:colors.border},inviteLabel:{fontSize:13,fontWeight:'700',color:colors.muted},inviteCode:{marginVertical:spacing.sm,fontSize:25,letterSpacing:2.5,fontWeight:'900',color:colors.roseDark},inviteHelp:{fontSize:13,lineHeight:19,color:colors.muted},sectionHeader:{gap:spacing.xs},sectionTitle:{fontSize:20,fontWeight:'900',color:colors.ink},sectionCaption:{fontSize:13,lineHeight:19,color:colors.muted},grid:{flexDirection:'row',flexWrap:'wrap',gap:spacing.md},feature:{width:'47.5%',minHeight:145,padding:spacing.md,borderRadius:radius.lg,backgroundColor:colors.surface,borderWidth:1,borderColor:colors.border},featureDisabled:{opacity:.65},featureIcon:{width:44,height:44,borderRadius:radius.md,alignItems:'center',justifyContent:'center',backgroundColor:colors.blush},featureTitle:{marginTop:spacing.md,fontSize:16,fontWeight:'800',color:colors.ink},featureCaption:{marginTop:spacing.xs,fontSize:13,lineHeight:18,color:colors.muted},backgroundFeatures:{gap:spacing.md,padding:spacing.lg,borderRadius:radius.lg,backgroundColor:colors.sageSoft,borderWidth:1,borderColor:colors.border},backgroundTitle:{fontSize:16,fontWeight:'800',color:colors.ink},backgroundRow:{flexDirection:'row',alignItems:'center',gap:spacing.sm},backgroundText:{flex:1,fontSize:13,lineHeight:19,color:colors.muted},disclaimer:{textAlign:'center',fontSize:12,lineHeight:18,color:colors.muted},errorTitle:{textAlign:'center',fontSize:20,fontWeight:'800',color:colors.ink},errorText:{maxWidth:340,textAlign:'center',fontSize:14,lineHeight:21,color:colors.muted},retryButton:{minWidth:130,minHeight:50,alignItems:'center',justifyContent:'center',borderRadius:radius.pill,backgroundColor:colors.rose},retryText:{fontWeight:'800',color:colors.surface}});