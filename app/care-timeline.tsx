...
export default function CareTimelineScreen() {
  ...
  function reset() {
    setEditingId(null); setType('doctor_visit'); setStatus('scheduled');
    setScheduledAt(new Date(Date.now() + 86400000)); setProvider(''); setFacility('');
    setPurpose(''); setQuestions(''); setNotes(''); setTests(''); setPickerMode(null);
  }

  function edit(item: CareAppointment) {
    setEditingId(item.id); setType(item.appointment_type); setStatus(item.status);
    setScheduledAt(new Date(item.scheduled_at)); setProvider(item.provider_name ?? '');
    setFacility(item.facility_name ?? ''); setPurpose(item.purpose ?? '');
    setQuestions(item.questions.join('\n')); setNotes(item.notes_after ?? '');
    setTests(item.tests_prescribed.join('\n'));
  }
  ...
  async function save() {
    ...
    try {
      await saveCareAppointment(pregnancyId, {
        ...
        notes_after: notes.trim() || null,
        tests_prescribed: splitLines(tests),
  ...
  return <SafeAreaView style={styles.page}>
    <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      <View style={styles.header}><Pressable onPress={() => router.back()} style={styles.back}><Ionicons name="arrow-back" size={22} color={colors.ink} /></Pressable><View style={{ flex: 1 }}><Text style={styles.eyebrow}>PRIVATE CARE TIMELINE</Text><Text style={styles.title}>Visits, scans and follow-ups in one place.</Text></View></View>
      ...
      <View style={styles.card}>
        <Text style={styles.cardTitle}>{editingId ? 'Edit care entry' : 'Add care entry'}</Text>
        <Text style={styles.label}>Type</Text><View style={styles.row}>{APPOINTMENT_TYPES.map((x) => <Pressable key={x.value} onPress={() => setType(x.value)} style={[styles.pill, type === x.value && styles.selected]}><Text style={styles.pillText}>{x.label}</Text></Pressable>)}</View>
        <View style={styles.row}><Pressable style={styles.dateButton} onPress={() => setPickerMode('date')}><Text style={styles.small}>Date</Text><Text style={styles.value}>{scheduledAt.toLocaleDateString()}</Text></Pressable><Pressable style={styles.dateButton} onPress={() => setPickerMode('time')}><Text style={styles.small}>Time</Text><Text style={styles.value}>{scheduledAt.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</Text></Pressable></View>
        <Field label="Doctor / provider" value={provider} onChangeText={setProvider} maxLength={120} />
        <Field label="Hospital / clinic / lab" value={facility} onChangeText={setFacility} maxLength={160} />
        <Field label="Purpose" value={purpose} onChangeText={setPurpose} maxLength={500} />
        <Field label="Questions to ask" value={questions} onChangeText={setQuestions} multiline placeholder="One per line" />
        <Text style={styles.label}>Status</Text><View style={styles.row}>{(['scheduled','completed','cancelled'] as AppointmentStatus[]).map((x) => <Pressable key={x} onPress={() => setStatus(x)} style={[styles.pill, status === x && styles.selected]}><Text style={styles.pillText}>{x}</Text></Pressable>)}</View>
        {status === 'completed' ? <><Field label="Notes after visit" value={notes} onChangeText={setNotes} multiline maxLength={4000} /><Field label="Tests / scans prescribed" value={tests} onChangeText={setTests} multiline placeholder="One per line" /></> : null}
        <View style={styles.row}>{editingId ? <Pressable onPress={reset} style={styles.secondary}><Text style={styles.secondaryText}>Cancel</Text></Pressable> : null}<Pressable disabled={saving} onPress={() => void save()} style={styles.primary}>{saving ? <ActivityIndicator color={colors.surface} /> : <Text style={styles.primaryText}>Save</Text>}</Pressable></View>
      </View>
      <View style={styles.card}><Text style={styles.cardTitle}>Timeline</Text>{items.length === 0 ? <Text style={styles.muted}>No entries yet.</Text> : items.map((item) => <View key={item.id} style={styles.entry}><View style={{ flex: 1 }}><Text style={styles.entryTitle}>{APPOINTMENT_TYPES.find((x) => x.value === item.appointment_type)?.label ?? 'Care'} · {item.status}</Text><Text style={styles.muted}>{new Date(item.scheduled_at).toLocaleString()}</Text>{item.purpose ? <Text style={styles.muted}>{item.purpose}</Text> : null}</View><Pressable onPress={() => edit(item)}><Ionicons name="create-outline" size={20} color={colors.roseDark} /></Pressable><Pressable onPress={() => remove(item)}><Ionicons name="trash-outline" size={20} color={colors.muted} /></Pressable></View>)}</View>
