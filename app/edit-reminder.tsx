...
export default function EditReminderScreen() {
  ...
  const [schedule,setSchedule]=useState<{kind:ReminderKind;startDate:string;endDate:string|null;daysOfWeek:number[]}|null>(null); const [loading,setLoading]=useState(true); const [saving,setSaving]=useState(false);

  useEffect(()=>{async function load(){const {data,error}=await supabase.from('reminders').select('title,instructions,kind,local_time,start_date,end_date,days_of_week').eq('id',id).single();if(error||!data){Alert.alert('Reminder unavailable',error?.message??'This reminder could not be found.');router.back();return;}const[h,m]=data.local_time.split(':').map(Number);setTitle(data.title);setInstructions(data.instructions??'');setTimeValue(new Date(2000,0,1,h,m));setSchedule({kind:data.kind as ReminderKind,startDate:data.start_date,endDate:data.end_date,daysOfWeek:data.days_of_week});setLoading(false);}if(id)load();},[id]);
