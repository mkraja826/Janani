...
export default function SettingsScreen() {
  ...
  useEffect(() => { void readGlobalUiLocale().then((next) => { setLocale(next); setCopy(formsAccountCopy(next)); }); }, []);
  ...
  useFocusEffect(useCallback(() => { void load(); void readGlobalUiLocale().then((next) => { setLocale(next); setCopy(formsAccountCopy(next)); }); }, [load]));
  ...
  return <SafeAreaView style={styles.page}><ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
    <View style={[styles.header, rtl.row]}><Pressable onPress={() => router.back()} style={styles.iconButton}><Ionicons name={directionalIconName(locale, 'arrow-back', 'arrow-forward') as keyof typeof Ionicons.glyphMap} size={22} color={colors.ink} /></Pressable><View style={styles.flex}><Text style={[styles.eyebrow, rtl.startText]}>{copy.settingsEyebrow}</Text><Text style={[styles.title, rtl.startText]}>{copy.settingsTitle}</Text><Text style={[styles.headerCaption, rtl.startText]}>Account, Care+, privacy and family controls.</Text></View></View>

    <Section title="Janani Care+" rtl={rtl.isRtl}>
      <Action rtl={rtl.isRtl} icon="card-outline" title="Billing & subscription" description="Check Care+ access and manage an existing Google Play subscription." onPress={() => router.push('/billing')} />
      <Action rtl={rtl.isRtl} icon="sparkles-outline" title="Janani Care+" description="Open personalised AI support when your account is eligible." onPress={() => router.push('/ai-companion')} />
    </Section>

    <Section title="Privacy & legal" rtl={rtl.isRtl}>
      <Action rtl={rtl.isRtl} icon="lock-closed-outline" title="Privacy Policy" description="Read Janani’s privacy policy inside the app." onPress={() => router.push('/privacy-policy')} />
      <Action rtl={rtl.isRtl} icon="document-text-outline" title="Terms of Service" description="Read Janani’s Terms of Use and medical disclaimer." onPress={() => router.push('/terms-of-service')} />
      <Action rtl={rtl.isRtl} icon="shield-checkmark-outline" title={copy.safetyPrivacy} description="Health-guidance boundaries, sharing choices, permissions and emergency guidance." onPress={() => router.push('/safety-privacy')} />
    </Section>
