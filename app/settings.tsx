...
export default function SettingsScreen() {
  ...
  return <SafeAreaView style={styles.page}><ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
    <View style={[styles.header, rtl.row]}><Pressable onPress={() => router.back()} style={styles.iconButton}><Ionicons name={directionalIconName(locale, 'arrow-back', 'arrow-forward') as keyof typeof Ionicons.glyphMap} size={22} color={colors.ink} /></Pressable><View style={styles.flex}><Text style={[styles.eyebrow, rtl.startText]}>{copy.settingsEyebrow}</Text><Text style={[styles.title, rtl.startText]}>{copy.settingsTitle}</Text>{session?.user.email ? <Text style={[styles.accountEmail, rtl.startText]}>{session.user.email}</Text> : null}</View></View>

    <Section title="Care+ & billing" rtl={rtl.isRtl}>
      <Action rtl={rtl.isRtl} icon="card-outline" title="Plan & billing" description="View Care+ subscription status, monthly AI allowance and Google Play billing information." onPress={() => router.push('/billing')} />
    </Section>
