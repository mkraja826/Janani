...
export default function OnboardingScreen() {
  ...
  const helper = useMemo(() => role === 'mother' ? tr('onboardingMotherHelper') : tr('onboardingPartnerHelper'), [role, language]);
  ...
  return <SafeAreaView style={styles.safeArea}><KeyboardAvoidingView style={styles.page} behavior={Platform.OS === 'ios' ? 'padding' : undefined}><ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
    <View style={styles.header}><Text style={styles.eyebrow}>{tr('onboardingEyebrow')}</Text><Text style={styles.title}>{role === 'mother' ? tr('onboardingMotherTitle') : tr('onboardingPartnerTitle')}</Text><Text style={styles.subtitle}>{helper}</Text></View>
    <View style={styles.roleRow}><RoleCard selected={role === 'mother'} icon="woman-outline" label={tr('mother')} onPress={() => setRole('mother')} /><RoleCard selected={role === 'partner'} icon="people-outline" label={tr('partner')} onPress={() => setRole('partner')} /></View>
    <View style={styles.form}>
      <Field label={tr('yourName')} value={fullName} onChangeText={setFullName} placeholder={tr('yourNamePlaceholder')} />
      {role === 'mother' ? <>
        <Field label={tr('familyName')} value={familyName} onChangeText={setFamilyName} placeholder={tr('familyNamePlaceholder')} />
        <DateButton label={tr('expectedDueDate')} value={dueDate} placeholder={tr('chooseDueDate')} onPress={() => setDateField('due')} />
        <DateButton label={tr('lastMenstrualPeriod')} value={lmp} placeholder={tr('chooseDate')} onPress={() => setDateField('lmp')} />
        <View style={styles.inlineFields}><View style={styles.flex}><Field label={tr('heightCm')} value={height} onChangeText={setHeight} placeholder="165" keyboardType="decimal-pad" /></View><View style={styles.flex}><Field label={tr('weightKg')} value={weight} onChangeText={setWeight} placeholder="60" keyboardType="decimal-pad" /></View></View>
      </> : <Field label={tr('familyInviteCode')} value={inviteCode} onChangeText={setInviteCode} placeholder={tr('inviteCodePlaceholder')} autoCapitalize="characters" autoCorrect={false} maxLength={20} />}
    </View>
    ...
    <Pressable disabled={busy || signingOut || deletingAccount} onPress={continueOnboarding} style={[styles.primary, (busy || signingOut || deletingAccount) && styles.disabled]}><Text style={styles.primaryText}>{busy ? tr('preparingSpace') : role === 'mother' ? tr('createFamilySpace') : tr('joinJourney')}</Text>{!busy && <Ionicons name="arrow-forward" size={20} color={colors.surface} />}</Pressable>
    <View style={styles.accountActions}><Text style={styles.accountHelp}>{tr('notReadyYet')}</Text><Pressable disabled={busy || signingOut || deletingAccount} onPress={() => void performSignOut()} style={[styles.secondary, (busy || signingOut || deletingAccount) && styles.disabled]}><Ionicons name="log-out-outline" size={19} color={colors.roseDark} /><Text style={styles.secondaryText}>{signingOut ? tr('signingOut') : tr('signOut')}</Text></Pressable></View>
    <View style={deletionStyles.section}><Text style={deletionStyles.title}>Delete this account</Text><Text style={deletionStyles.help}>If you do not want to create or join a family, you can permanently remove this account now.</Text><TextInput autoCapitalize="characters" autoCorrect={false} placeholder="Type DELETE" placeholderTextColor={colors.muted} style={deletionStyles.input} value={deleteText} onChangeText={setDeleteText} /><TextInput autoCapitalize="none" autoComplete="current-password" secureTextEntry placeholder="Current password" placeholderTextColor={colors.muted} style={deletionStyles.input} value={currentPassword} onChangeText={setCurrentPassword} /><Pressable disabled={busy || signingOut || deletingAccount} onPress={confirmAccountDeletion} style={[deletionStyles.button, (busy || signingOut || deletingAccount) && styles.disabled]}><Ionicons name="trash-outline" size={18} color={colors.surface} /><Text style={deletionStyles.buttonText}>{deletingAccount ? 'Deleting account…' : 'Delete account permanently'}</Text></Pressable></View>
