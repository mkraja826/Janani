...
export default function OnboardingScreen() {
  ...
  const intendedRole = params.role === 'partner' || session?.user.user_metadata?.intended_role === 'partner'
    ? 'partner'
    : 'mother';
  ...
  async function continueOnboarding() {
    ...
    if (name.length < 2 || name.length > 80) {
      return Alert.alert('Tell Janani your name', 'Enter a name between 2 and 80 characters.');
    }
    if (role === 'mother' && !dueDate) return Alert.alert('Due date needed', 'Choose the expected due date.');
    if (role === 'mother' && normalizedFamilyName.length > 80) {
      return Alert.alert('Family name is too long', 'Keep the family name within 80 characters.');
    }
    if (role === 'mother' && lmp && dueDate && lmp > dueDate) {
      return Alert.alert('Check the dates', 'The last menstrual period must be before the expected due date.');
    }
    if (role === 'mother' && heightCm !== null && (!Number.isFinite(heightCm) || heightCm < 80 || heightCm > 250)) {
      return Alert.alert('Check height', 'Enter a height between 80 and 250 cm, or leave it blank.');
    }
    if (role === 'mother' && weightKg !== null && (!Number.isFinite(weightKg) || weightKg < 25 || weightKg > 300)) {
      return Alert.alert('Check weight', 'Enter a weight between 25 and 300 kg, or leave it blank.');
    }
    if (role === 'partner' && !/^[A-F0-9]{20}$/.test(normalizedInviteCode)) {
      return Alert.alert('Invite code needed', 'Enter the 20-character code shared from the mother’s Janani app.');
    }
    ...
    try {
      const result = role === 'mother'
        ? await supabase.rpc('create_mother_family', {
            p_full_name: name, p_family_name: normalizedFamilyName,
            p_due_date: dateOnly(dueDate!), p_last_menstrual_period: lmp ? dateOnly(lmp) : null,
            p_height_cm: heightCm, p_pre_pregnancy_weight_kg: weightKg,
          })
      ...
      const pregnancyId = firstRow
        && typeof firstRow === 'object'
        && 'pregnancy_id' in firstRow
        && typeof firstRow.pregnancy_id === 'string'
        ? firstRow.pregnancy_id
        : undefined;
      if (session && pregnancyId) {
        await cacheActivePregnancyId(session.user.id, pregnancyId);
      }
      const createdFamilyId = firstRow
        && typeof firstRow === 'object'
        && 'family_id' in firstRow
        && typeof firstRow.family_id === 'string'
        ? firstRow.family_id
        : undefined;
      await markMembership(true, createdFamilyId);
      const createdCode = role === 'mother'
        && firstRow
        && typeof firstRow === 'object'
        && 'invite_code' in firstRow
        && typeof firstRow.invite_code === 'string'
        ? firstRow.invite_code
        : undefined;
      if (createdCode) {
        Alert.alert(
          'Your family is ready',
          `Partner invite code: ${createdCode}`,
      ...
      Alert.alert('Could not complete setup', 'Check your connection and try again.');
    } finally {
      setBusy(false);
    }
  }
  ...
  async function performSignOut(discardPending = false) {
    ...
    try {
      ...
      if (error instanceof PendingOfflineChangesError) {
        Alert.alert(
          'Unsynced changes',
          `${error.count} offline change${error.count === 1 ? '' : 's'} will be lost if you sign out now.`,
          [
            { text: 'Keep me signed in', style: 'cancel' },
            {
              text: 'Discard and sign out',
              style: 'destructive',
              onPress: () => void performSignOut(true),
            },
          ],
        );
      } else {
  ...
  function confirmAccountDeletion() {
    if (deleteText !== 'DELETE' || !currentPassword) {
      Alert.alert('Confirmation needed', 'Type DELETE exactly and enter your current password.');
      return;
    }
    Alert.alert(
      'Permanently delete account?',
      'This permanently deletes your Janani account. This cannot be undone.',
      [
  ...
  async function deleteAccount() {
    ...
    try {
      ...
      setCurrentPassword('');
      setDeleteText('');
      await signOut({ discardPending: true }).catch(async () => {
  ...
  return <SafeAreaView style={styles.safeArea}>
    ...
    <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      <View style={styles.form}>
        ...
        </> : <Field label="Family invite code" value={inviteCode} onChangeText={setInviteCode} placeholder="20-character invite code" autoCapitalize="characters" autoCorrect={false} maxLength={20} />}
      </View>
      ...
      <Pressable disabled={busy || signingOut || deletingAccount} onPress={continueOnboarding} style={[styles.primary, (busy || signingOut || deletingAccount) && styles.disabled]}><Text style={styles.primaryText}>{busy ? 'Preparing your space…' : role === 'mother' ? 'Create our family space' : 'Join her journey'}</Text>{!busy && <Ionicons name="arrow-forward" size={20} color={colors.surface} />}</Pressable>
      <View style={styles.accountActions}>
        <Text style={styles.accountHelp}>Not ready to create or join a family yet?</Text>
        <Pressable
          disabled={busy || signingOut || deletingAccount}
          onPress={() => void performSignOut()}
          style={[styles.secondary, (busy || signingOut || deletingAccount) && styles.disabled]}
        >
          <Ionicons name="log-out-outline" size={19} color={colors.roseDark} />
          <Text style={styles.secondaryText}>{signingOut ? 'Signing out…' : 'Sign out'}</Text>
        </Pressable>
      </View>
      <View style={deletionStyles.section}>
        <Text style={deletionStyles.title}>Delete this account</Text>
        <Text style={deletionStyles.help}>If you do not want to create or join a family, you can permanently remove this account now.</Text>
        <TextInput
          autoCapitalize="characters"
          autoCorrect={false}
          placeholder="Type DELETE"
          placeholderTextColor={colors.muted}
          style={deletionStyles.input}
          value={deleteText}
        ...
        <Pressable
          ...
          style={[deletionStyles.button, (busy || signingOut || deletingAccount) && styles.disabled]}
        >
          <Ionicons name="trash-outline" size={18} color={colors.surface} />
          <Text style={deletionStyles.buttonText}>{deletingAccount ? 'Deleting account…' : 'Delete account permanently'}</Text>
        </Pressable>
      </View>
    </ScrollView>
