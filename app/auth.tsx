...
export default function AuthScreen() {
  ...
  async function submit() {
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(normalizedEmail) || !password) return Alert.alert('A small check', 'Enter a valid email and password.');
    if (mode === 'sign-up' && (password.length < 8 || !/[A-Za-z]/.test(password) || !/\d/.test(password))) return Alert.alert('Choose a stronger password', 'Use at least 8 characters with both a letter and a number.');

    setBusy(true);
    try {
      const action = mode === 'sign-up'
        ? supabase.auth.signUp({ email: normalizedEmail, password, options: { data: { intended_role: params.role === 'partner' ? 'partner' : 'mother' }, emailRedirectTo: EMAIL_CONFIRMATION_REDIRECT } })
        : supabase.auth.signInWithPassword({ email: normalizedEmail, password });
      const { data, error } = await withTimeout(action);
      if (error) { Alert.alert('Could not continue', error.message); return; }
      if (mode === 'sign-up' && !data.session) { Alert.alert('Check your email', 'Open the confirmation email. PregaLove will finish signing you in after you confirm your address.'); setMode('sign-in'); return; }
      const user = data.user;
      if (!user) { Alert.alert('Could not continue', 'PregaLove could not verify this account. Please sign in again.'); return; }
      const { data: membership, error: membershipError } = await withTimeout(supabase.from('family_members').select('family_id').eq('user_id', user.id).maybeSingle());
      if (membershipError) { Alert.alert('Could not verify your family', 'Check your connection and try again.'); return; }
      if (membership) { await markMembership(true, membership.family_id); router.replace('/home'); return; }
      await markMembership(false);
      const role = params.role === 'partner' || user.user_metadata?.intended_role === 'partner' ? 'partner' : 'mother';
      router.replace(role === 'mother' ? '/onboarding-ai' : { pathname: '/onboarding', params: { role } });
    } catch (error) {
      if (error instanceof Error && error.message === 'AUTH_REQUEST_TIMEOUT') return Alert.alert('Connection timed out', 'PregaLove could not reach the sign-in service. Check your internet connection and try again.');
  ...
  return <SafeAreaView style={styles.safeArea}><KeyboardAvoidingView style={styles.page} behavior={Platform.OS === 'ios' ? 'padding' : undefined}><View style={[styles.header, rtl.startAligned]}><View style={styles.icon}><Ionicons name="heart" size={28} color={colors.rose} /></View><Text style={[styles.eyebrow, rtl.startText]}>{tr('authWelcome')}</Text><Text style={[styles.title, rtl.startText]}>{mode === 'sign-up' ? tr('authCreateTitle') : tr('authSignInTitle')}</Text><Text style={[styles.subtitle, rtl.startText]}>{tr('authSubtitle')}</Text></View><View style={styles.form}><TextInput autoCapitalize="none" autoComplete="email" keyboardType="email-address" placeholder={tr('emailAddress')} placeholderTextColor={colors.muted} style={[styles.input, rtl.startText]} value={email} onChangeText={setEmail} /><TextInput autoCapitalize="none" autoComplete={mode === 'sign-up' ? 'new-password' : 'current-password'} secureTextEntry placeholder={tr('password')} placeholderTextColor={colors.muted} style={[styles.input, rtl.startText]} value={password} onChangeText={setPassword} /><Pressable disabled={busy} onPress={submit} style={[styles.primary, busy && styles.disabled]}><Text style={styles.primaryText}>{busy ? tr('pleaseWait') : mode === 'sign-up' ? tr('createAccount') : tr('signIn')}</Text></Pressable><Pressable onPress={() => setMode(mode === 'sign-up' ? 'sign-in' : 'sign-up')} style={styles.switchButton}><Text style={[styles.switchText, rtl.text]}>{mode === 'sign-up' ? tr('alreadyRegistered') : tr('newToJanani')}</Text></Pressable></View></KeyboardAvoidingView></SafeAreaView>;
}
