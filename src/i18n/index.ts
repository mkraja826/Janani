...
export function t(language: JananiLanguage, key: MessageKey): string { return messages[language][key] ?? en[key]; }
export async function readUiLanguage(): Promise<JananiLanguage> { const value = await AsyncStorage.getItem(STORAGE_KEY); return value === 'te' || value === 'hi' ? value : 'en'; }
export async function writeUiLanguage(language: JananiLanguage): Promise<void> { await AsyncStorage.setItem(STORAGE_KEY, language); }
