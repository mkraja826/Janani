...
export type HealthCondition = {
  condition_code: HealthConditionCode;
...
export type SaveHealthProfileInput = Omit<HealthProfile, 'pregnancy_id' | 'conditions'> & {
  conditions: HealthCondition[];
};
...
export async function loadHealthProfile(pregnancyId: string): Promise<HealthProfile> {
  const { data, error } = await healthRpc<HealthProfile>('get_own_health_profile', {
