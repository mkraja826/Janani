...
function safeDatabaseError(error: { code?: string }): PublicError {
  if (error.code === "28000") {
    return new PublicError(401, "Authentication required");
  }
  if (error.code === "42501") {
    return new PublicError(403, "This request is not permitted");
  }
  if (error.code === "22023") {
    return new PublicError(400, "The message request is invalid");
  }
  if (error.code === "P0001") {
    return new PublicError(429, "Please wait before sending another message");
  }
...
async function sendPushNotifications(
  ...
  for (let offset = 0; offset < tokens.length; offset += EXPO_BATCH_SIZE) {
    const tokenChunk = tokens.slice(offset, offset + EXPO_BATCH_SIZE);
    const payloads = tokenChunk.map((token) => ({
      to: token,
      sound: "default",
      channelId: "janani-partner-messages",
      title: "Someone is thinking of you",
      body: "Open Janani to view a private partner note.",
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Expo push status ${response.status}`);
      }
      ...
      tickets.forEach((ticket, index) => {
        // "ok" means Expo accepted the notification for processing; it does
        // not prove device delivery. Delivery requires receipt polling.
      ...
      // The nudge is already durably stored. A push outage must not turn a
      // successful request into a duplicate-producing client retry.
...
Deno.serve(async (request: Request): Promise<Response> => {
  ...
  try {
    ...
    const suppliedMutationId = body?.client_mutation_id ??
      body?.clientMutationId ??
      request.headers.get("idempotency-key");
    if (
      suppliedMutationId !== null &&
      suppliedMutationId !== undefined &&
      (typeof suppliedMutationId !== "string" ||
        !UUID_PATTERN.test(suppliedMutationId))
    ) {
      throw new PublicError(400, "Idempotency key must be a UUID");
    }
    const mutationId = typeof suppliedMutationId === "string"
      ? suppliedMutationId
      : crypto.randomUUID();
    ...
    // Only one invocation may claim the push attempt for this durable nudge.
    // A replay still returns the same nudge id but never sends another push.
    ...
    const { data: tokenRows, error: tokenError } = await admin
      ...
      .limit(20);
    if (tokenError) {
      logFailure(requestId, "load-push-tokens", tokenError);
    }

    const tokens = (tokenRows ?? [])
      .map((row) => row.expo_push_token)
      .filter((token): token is string => typeof token === "string");
    const acceptedByExpo = tokenError ? 0 : await sendPushNotifications(
      admin,
      tokens,
      nudge,
      requestId,
    ...
    return jsonResponse(
      {
        ok: true,
        nudge_id: nudge.id,
        // Retain the current response field without overstating delivery.
        delivered_to: 0,
