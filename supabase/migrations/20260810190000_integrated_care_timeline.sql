...
create table if not exists public.care_appointments (
  ...
  check (cardinality(questions) <= 20),
  check (cardinality(tests_prescribed) <= 20)
);

create index if not exists care_appointments_pregnancy_time_idx on public.care_appointments(pregnancy_id, scheduled_at desc);
create index if not exists care_appointments_mother_id_idx on public.care_appointments(mother_id);
...
create or replace function public.normalize_short_text_array(p_values jsonb, p_max_items integer default 20, p_max_chars integer default 180)
returns text[]
language sql
immutable
as $$
  select coalesce(array_agg(value order by first_ordinal), '{}')
  from (
    select value, min(ord) as first_ordinal
    from (
      select left(btrim(value), greatest(1, p_max_chars)) as value, ord
      from jsonb_array_elements_text(coalesce(p_values, '[]'::jsonb)) with ordinality as e(value, ord)
      where btrim(value) <> ''
      order by ord
      limit greatest(0, least(p_max_items, 50))
    ) normalized
    group by value
  ) deduped;
$$;
...
begin
  ...
  into v_result from public.care_appointments a
  where a.pregnancy_id = p_pregnancy_id and a.mother_id = v_user_id;
  return v_result;
end;
$$;

create or replace function public.save_own_care_appointment(p_pregnancy_id uuid, p_appointment jsonb)
returns jsonb
language plpgsql
security definer
set search_path = public
...
begin
  ...
  if v_id is null then
    insert into public.care_appointments(
      pregnancy_id,mother_id,appointment_type,scheduled_at,provider_name,facility_name,purpose,
      questions,notes_after,tests_prescribed,next_followup_at,status
    ) values (
      p_pregnancy_id,v_user_id,coalesce(nullif(p_appointment->>'appointment_type',''),'doctor_visit'),v_scheduled_at,
      nullif(left(btrim(coalesce(p_appointment->>'provider_name','')),120),''),
      nullif(left(btrim(coalesce(p_appointment->>'facility_name','')),160),''),
      nullif(left(btrim(coalesce(p_appointment->>'purpose','')),500),''),
      public.normalize_short_text_array(p_appointment->'questions',20,300),
      nullif(left(btrim(coalesce(p_appointment->>'notes_after','')),4000),''),
      public.normalize_short_text_array(p_appointment->'tests_prescribed',20,220),
      v_followup_at,coalesce(nullif(p_appointment->>'status',''),'scheduled')
    ...
    update public.care_appointments set
      ...
      provider_name = nullif(left(btrim(coalesce(p_appointment->>'provider_name','')),120),''),
      facility_name = nullif(left(btrim(coalesce(p_appointment->>'facility_name','')),160),''),
      purpose = nullif(left(btrim(coalesce(p_appointment->>'purpose','')),500),''),
      questions = public.normalize_short_text_array(p_appointment->'questions',20,300),
      notes_after = nullif(left(btrim(coalesce(p_appointment->>'notes_after','')),4000),''),
      tests_prescribed = public.normalize_short_text_array(p_appointment->'tests_prescribed',20,220),
...
create or replace function public.delete_own_care_appointment(p_pregnancy_id uuid, p_appointment_id uuid)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  ...
  delete from public.care_appointments where id = p_appointment_id and pregnancy_id = p_pregnancy_id and mother_id = v_user_id;
end;
$$;

revoke all on function public.normalize_short_text_array(jsonb,integer,integer) from public;
